...
export class IdeaController {
    ...
    @Post(':id/upvote')
    @UseGuards(new AuthGuard())
    upvoteIdea(@Param('id') id: number, @User('id') userId: number) {
      return this.ideaService.upvote(id, userId);
    }
  
    @Post(':id/downvote')
    @UseGuards(new AuthGuard())
    downvoteIdea(@Param('id') id: number, @User('id') userId: number) {
      return this.ideaService.downvote(id, userId);
    }

    @Post(':id/bookmark')
    @UseGuards(new AuthGuard())
    bookmarkIdea(@Param('id') id: number, @User('id') userId: number){
        return this.ideaService.bookmark(id, userId);
    }

    @Delete(':id/bookmark')
    @UseGuards(new AuthGuard())
    unbookmarkIdea(@Param('id') id: number, @User('id') userId: number){
